...
if [ $# -le 0 ]
    then echo "A pcap file should be provided to this script. Example: ./scripts/scriptname.sh file.pcap [num of queues, default=${TXQUEUES}] [rate_limit_mbps]"
    echo "Optional rate_limit_mbps parameter limits transmission rate (e.g., 100 for 100 Mbps)"
    exit 1
fi
PCAP_FILE="$1"
...
# Prepare rate limiting parameter
BWL_PARAM=""
if [ ! -z "$RATE_LIMIT" ]; then
    BWL_PARAM="--bwl ${RATE_LIMIT}"
    echo "Rate limiting enabled: ${RATE_LIMIT} Mbps"
else
    echo "Rate limiting disabled (unlimited transmission)"
fi
